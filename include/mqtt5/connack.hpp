...
[[nodiscard]] Iter deserialize_into(connack::reason &r, Iter begin, Iter end) {
    ...
    r = static_cast<connack::reason>(code);
    return begin;
}

template <class Iter>
[[nodiscard]] Iter deserialize_into(connack &c, Iter begin, Iter end) {
    using mqtt5::type::string;
    using mqtt5::type::integer8;
    using mqtt5::type::integer16;
    using mqtt5::type::integer32;
    
    begin = mqtt5::deserialize_into(c.session_present, begin, end);
    begin = mqtt5::deserialize_into(c.reason_code, begin, end);

    std::vector<mqtt5::property> properties;
    begin = mqtt5::deserialize_into(properties, begin, end);
    for (const auto &prop : properties) {
        ...
        case property_id::retain_available:
            c.retain_available = std::get<integer8>(prop.value);
            break;
        ...
        case property_id::wildcard_subscription_available:
            c.wildcard_subscription_available = std::get<integer8>(prop.value);
            break;
        case property_id::subscription_identifiers_available:
            c.subscription_identifiers_available = std::get<integer8>(prop.value);
            break;
        case property_id::shared_subscription_available:
            c.shared_subscription_available = std::get<integer8>(prop.value);
            break;
