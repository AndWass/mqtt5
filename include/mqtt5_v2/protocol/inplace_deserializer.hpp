...
struct optional_inplace_deserializer_sender
{
    ...
    struct operation
    {
        using next_sender_type = decltype(std::declval<T&>().inplace_deserializer(std::declval<data_fetcher_type>()));
        ...
        void start() {
            if(val_->has_value()) {
                ...
                auto &next = next_.template emplace<1>(p0443_v2::connect(ref.inplace_deserializer(data_fetcher_), std::move(recv_)));
                p0443_v2::start(next);
            }
            else {
...
template<class T, class Stream>
auto inplace_deserializer(T &val, transport::data_fetcher<Stream> data_fetcher) -> decltype(val.inplace_deserializer(data_fetcher)) {
