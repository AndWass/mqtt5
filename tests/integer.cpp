...
TEST_CASE("integer16: deserialize_into failure") {
    ...
    mqtt5::type::integer16 integer;
    REQUIRE_THROWS_AS((void)deserialize_into(integer, data.begin(), data.end()),
                      boost::system::system_error);
    SUBCASE("empty buffer") {
        std::vector<std::uint8_t> empty;
        mqtt5::type::integer16 integer;
        REQUIRE_THROWS_AS((void)deserialize_into(integer, empty.begin(), empty.end()),
                          boost::system::system_error);
    }
}
...
TEST_CASE("integer8 stream read") {
    ...
    mqtt5::type::integer8 value;
    boost::system::error_code error;
    mqtt5::type::integer_from_stream<mqtt5::type::integer8>(
        rx_stream, value_receiver{value, error});

    io.run();
    REQUIRE(value.value() == data[0]);
    REQUIRE_FALSE(error);
    REQUIRE(rx_stream.nread_bytes() == 1);
}

TEST_CASE("integer32 stream read") {
    boost::asio::io_context io;
    boost::beast::test::stream tx_stream(io);
    boost::beast::test::stream rx_stream(io);
    rx_stream.connect(tx_stream);

    std::uint8_t data[4]{0xab, 0xbc, 0xcd, 0xde};
    tx_stream.write_some(boost::asio::buffer(data));
    mqtt5::type::integer32 value;
    boost::system::error_code error;
    mqtt5::type::integer_from_stream<mqtt5::type::integer32>(
        rx_stream, value_receiver{value, error});

    io.run();
    REQUIRE(value.value() == 0xabbccdde);
    REQUIRE_FALSE(error);
...
TEST_CASE("varlen_integer stream read") {
    ...
    SUBCASE("maximum value") {
        std::uint8_t data[4]{0xff, 0xff, 0xff, 0x7f};
        tx_stream.write_some(boost::asio::buffer(data));
        mqtt5::type::varlen_integer value;
        boost::system::error_code error;
        mqtt5::type::integer_from_stream<mqtt5::type::varlen_integer>(
            rx_stream, value_receiver{value, error});
