...
p0443_v2::immediate_task mqtt_task(net::io_context &io) {
    ...
    std::cout << co_await p0443_v2::await_sender(connection.control_packet_writer(connect))
              << " bytes written\n";
    ...
    if (connack && connack->reason_code == 0) {
        ...
        publish.set_packet_identifier(100);
